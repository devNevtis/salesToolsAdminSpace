...
export const PBXSettingsTab = () => {
  ...
  const handleDomainChange = (domainUuid) => {
    const domain = domains.find(d => d.value.domain_uuid === domainUuid);
    if (domain) {
      // Asegurarnos de que actualizamos el campo con el objeto completo
      form.setValue("pbxUrl", domain.value, {
        shouldValidate: true,
        shouldDirty: true,
        shouldTouch: true
      });
              ...
              <FormControl>
                <SelectTrigger className="h-8 text-sm">
                  <SelectValue placeholder="Select a PBX domain">
                    {selectedDomain?.domain_name || "Select a PBX domain"}
                  </SelectValue>
                </SelectTrigger>
              </FormControl>
              <SelectContent>
